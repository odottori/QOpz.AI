...
class TestF6T2UniverseApi(unittest.TestCase):
    ...
    def test_manual_scan_persists_settings_path_in_latest(self):
        manual_settings = Path("docs/test_ibkr_settings.xml")
        manual_settings.write_text("""<root><QuoteElement symbol="AAPL" exchange="SMART" /></root>""", encoding="utf-8")
        ...
        payload = {
            ...
            "top_n": 2,
            "source": "manual",
            "settings_path": str(manual_settings),
        }
        s = self.client.post("/opz/universe/scan", json=payload)
        self.assertEqual(s.status_code, 200, s.text)
        scan = s.json()
        self.assertEqual(Path(scan.get("ibkr_settings_path")).name, manual_settings.name)
        self.assertTrue(scan.get("ibkr_settings_exists"))
        ...
        self.assertEqual(latest.get("batch_id"), scan.get("batch_id"))
        self.assertEqual(latest.get("source"), "manual")
        self.assertEqual(Path(latest.get("ibkr_settings_path")).name, manual_settings.name)
        self.assertTrue(latest.get("ibkr_settings_exists"))
